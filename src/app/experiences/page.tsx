...
const EMOJI: Record<string,string> = {
  MEDINA_SECRETS:"ïå", GASTRONOMIE:"çΩÔ∏è", HISTOIRE_MONUMENTS:"èõÔ∏è",
  DESERT_NATURE:"èúÔ∏è", SHOPPING_ARTISANAT:"õçÔ∏è", COUCHER_SOLEIL:"åÖ", PHOTO_INSTAGRAM:"ì∏"
};
...
export default function ExperiencesPage() {
  ...
  const CITIES = ["Marrakech", "F√®s", "Essaouira", "Chefchaouen", "Agadir"];
              ...
              <Link key={t.id} href={t.isGuideExperience ? "/booking/" + t.guideId + "?expId=" + t.expId + "&tourPrice=" + Math.ceil(t.minPrice * 1.25) + (t.groupThreshold1 ? "&t1=" + t.groupThreshold1 + "&d1=" + t.groupDiscount1 : "") + (t.groupThreshold2 ? "&t2=" + t.groupThreshold2 + "&d2=" + t.groupDiscount2 : "") : "/experiences/" + t.tourType.toLowerCase()}
                ...
                <div className="p-4">
                  ...
                  <button onClick={(e) => { e.preventDefault(); setOpenId(openId === t.id ? null : t.id); }}
                    ...
                    {openId === t.id ? "Masquer ‚Üë" : "Voir details ‚Üì"}
                  </button>
                        ...
                        <div className="mb-2">
                          ...
                          {t.included.map((item:string) => (
                            <div key={item} className="flex items-center gap-1.5 mb-1">
                              <span className="text-sage-300 text-xs font-bold">‚úì</span>
                      ...
                      {t.notIncluded?.length > 0 && (
                        <div>
                          ...
                          {t.notIncluded.map((item:string) => (
                            <div key={item} className="flex items-center gap-1.5 mb-1">
                              <span className="text-red-400 text-xs font-bold">‚úó</span>
                              <span className="text-xs text-charcoal-600">{item}</span>
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <PriceDisplay mad={priceWithCommission(t.minPrice)} size="md" /> <span className="text-xs font-normal text-charcoal-400">{t.isGuideExperience ? {t.groupThreshold1 ? ("/ pers. ¬∑ -" + t.groupDiscount1 + "% d√®s " + t.groupThreshold1 + " pers.") : "/ pers."} : "/ 4 pers."}</span>
                          </div>
                        </>
                      ) : (
                    ...
                    <div className="flex items-center gap-1.5 bg-sage-300 text-white font-bold px-4 py-2.5 rounded-full text-xs">
                      {t.isGuideExperience ? "R√©server maintenant" : "Voir les guides"} <ArrowRight size={12} weight="bold" />
                    </div>
                  </div>
                </div>
              </Link>
