  ...
  return (
    <div className="min-h-screen pb-10" style={{background:"#F6F1E8"}}>
      <div className="flex items-center justify-between px-4 py-3 sticky top-0 z-30"
        ...
        <span className="font-display text-sm font-bold text-charcoal-800">Modifier l'expérience</span>
        <div className="w-9" />
      </div>
      ...
      <div className="mx-4 mt-4 rounded-2xl p-3 flex items-center gap-2 mb-4"
        ...
        <span className="text-xs text-bronze-500 font-semibold">Les modifications seront soumises pour validation</span>
      </div>
      ...
      <div className="px-4 flex flex-col gap-4 max-w-lg mx-auto">
        <Section title="Infos generales" icon={<ListBullets size={14} weight="bold" />}>
          <Field label="Titre *">
            <input value={form.title} onChange={set("title")} placeholder="Titre" className={inputCls} />
          </Field>
        ...
        <Section title="Logistique" icon={<MapPin size={14} weight="bold" />}>
          <Field label="Point de rendez-vous">
            <input value={form.meetingPoint} onChange={set("meetingPoint")} placeholder="Place Jemaa el-Fna" className={inputCls} />
          </Field>
          <Field label="Itineraire (HH:mm | Titre | Description)">
            <textarea value={form.itinerary_raw} onChange={set("itinerary_raw")} rows={4} className={inputCls + " resize-none font-mono text-xs"} />
          </Field>
        </Section>

        <Section title="Contenu et Tags" icon={<Tag size={14} weight="bold" />}>
          <Field label="Inclus (virgule)">
            <input value={form.included} onChange={set("included")} placeholder="Transport, Guide" className={inputCls} />
          </Field>
          <Field label="Non inclus (virgule)">
            <input value={form.notIncluded} onChange={set("notIncluded")} placeholder="Boissons alcoolisees" className={inputCls} />
          </Field>
          <Field label="Tags (virgule)">
            <input value={form.tags} onChange={set("tags")} placeholder="Desert, Aventure" className={inputCls} />
          </Field>
        </Section>
        ...
        <button onClick={save} disabled={saving}
          ...
          {saving ? "Enregistrement..." : "Soumettre les modifications"}
        </button>
      </div>
    </div>
  );
