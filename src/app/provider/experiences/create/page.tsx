...
export default function ProviderCreateExperience() {
  ...
  async function save() {
    ...
    const data: any = {
      ...
      status: "PENDING",
      isActive: false,
      guideId: "cmq5fr4ef0002xbtvwrfquu46",
      isLaksorExp: true,
    };
  ...
  return (
    <div className="min-h-screen pb-10" style={{background:"#F6F1E8"}}>
      <div className="flex items-center justify-between px-4 py-3 sticky top-0 z-30"
        ...
        <span className="font-display text-sm font-bold text-charcoal-800">Nouvelle expérience</span>
        <div className="w-9" />
      </div>
      ...
      <div className="mx-4 mt-4 rounded-2xl p-3 flex items-center gap-2 mb-4"
        ...
        <span className="text-xs text-bronze-500 font-semibold">Votre expérience sera publiée après validation par notre équipe</span>
      </div>
      ...
      <div className="px-4 flex flex-col gap-4 max-w-lg mx-auto">
        ...
        <Section title="Infos générales" icon={<ListBullets size={14} weight="bold" />}>
          <Field label="Titre *">
            <input value={form.title} onChange={set("title")} placeholder="Agafay Desert + Quad + Dîner" className={inputCls} />
          </Field>
          <Field label="Description">
            <textarea value={form.description} onChange={set("description")} placeholder="Décrivez votre expérience..." rows={3} className={inputCls + " resize-none"} />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Ville">
              <select value={form.city} onChange={set("city")} className={inputCls}>
                {["Marrakech","Fès","Essaouira","Chefchaouen","Agadir"].map(c => <option key={c}>{c}</option>)}
              </select>
            </Field>
            <Field label="Durée">
        ...
        <Section title="Prix" icon={<CurrencyDollar size={14} weight="bold" />}>
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Prix privé (MAD/pers.)">
              <input value={form.privatePricePerPerson} onChange={set("privatePricePerPerson")} placeholder="600" className={inputCls} />
            </Field>
          </div>
        </Section>

        <Section title="Créneaux & Langues" icon={<Clock size={14} weight="bold" />}>
          <div>
            <div className="text-xs font-semibold text-charcoal-400 mb-2">Créneaux de départ</div>
                ...
                {form.departureSlots.map((slot, i) => (
                  <div key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold"
                    ...
                    <button onClick={() => setForm(f => ({...f, departureSlots: f.departureSlots.filter((_,j) => j !== i)}))}
                      className="ml-1 text-red-400 font-bold">×</button>
                  </div>
                ))}
              </div>
            )}
        ...
        <Section title="Logistique" icon={<MapPin size={14} weight="bold" />}>
          ...
          <Field label="Itinéraire (HH:mm | Titre | Description)">
            <textarea value={form.itinerary_raw} onChange={set("itinerary_raw")}
              placeholder={"16:30 | Prise en charge | Transfert hôtel
17:30 | Arrivée | Accueil"} rows={4} className={inputCls + " resize-none font-mono text-xs"} />
          </Field>
        </Section>

        <Section title="Contenu & Tags" icon={<Tag size={14} weight="bold" />}>
          <Field label="Inclus (virgule)">
            <input value={form.included} onChange={set("included")} placeholder="Transport, Guide, Thé menthe" className={inputCls} />
          </Field>
          <Field label="Non inclus (virgule)">
            <input value={form.notIncluded} onChange={set("notIncluded")} placeholder="Boissons alcoolisées, Pourboires" className={inputCls} />
          </Field>
          <Field label="Tags (virgule)">
            <input value={form.tags} onChange={set("tags")} placeholder="Désert, Aventure, Couple" className={inputCls} />
          </Field>
        </Section>
