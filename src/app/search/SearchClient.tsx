...
function GuideCard({ g, idx }: { g: Guide; idx: number }) {
  ...
  return (
    <a href={`/guide/${g.id}`} style={{ textDecoration: "none", display: "block" }}>
      <div
        style={{ background: "var(--white)", borderRadius: 28, overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.07)", border: "1px solid rgba(234,220,200,0.7)", display: "flex", transition: "transform 0.18s, box-shadow 0.18s" }}
      ...
      >
        ...
        <div style={{ position: "relative", width: 120, flexShrink: 0 }}>
          <img src={g.avatar ?? "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=300&q=80"} alt={g.displayName} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", minHeight: 200 }} />
        ...
        <div style={{ flex: 1, padding: "18px 18px 16px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 9, color: "var(--muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 2 }}>📍 {g.city}</div>
            <div style={{ fontFamily: "var(--font-serif)", fontSize: 19, fontWeight: 700, color: "var(--charcoal)", marginBottom: 6, lineHeight: 1.15 }}>{g.displayName}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
              <StarRow rating={g.avgRating} />
              <span style={{ fontWeight: 800, fontSize: 12 }}>{g.avgRating.toFixed(1)}</span>
            ...
            {g.specialties.length > 0 && (
              <div style={{ fontSize: 11, color: "var(--soft)", marginBottom: 8, lineHeight: 1.4 }}>{g.specialties.slice(0,2).join(" · ")}</div>
            )}
            ...
            {g.visitTypes.length > 0 && (
              <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                {g.visitTypes.slice(0,3).map(t => (
                  <span key={t} style={{ fontSize: 12 }}>{MOOD_ICONS[t] ?? "✦"}</span>
                ))}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: 14, paddingTop: 12, borderTop: "1px solid var(--sand)" }}>
            <div>
              ...
              <div style={{ fontFamily: "var(--font-serif)", fontSize: 24, fontWeight: 700, color: "var(--charcoal)", lineHeight: 1 }}>€{eurPrice}</div>
              <div style={{ fontSize: 9, color: "var(--muted)", marginTop: 2 }}>/ pers · 4h privé</div>
            </div>
            <a href={`/booking?guide=${g.id}`} onClick={e => e.stopPropagation()} style={{ background: "var(--bronze-g)", color: "#fff", borderRadius: "999px", padding: "11px 20px", fontSize: 12, fontWeight: 700, textDecoration: "none", boxShadow: "0 4px 14px rgba(184,138,68,0.35)", flexShrink: 0 }}>
              Réserver →
            </a>
          </div>
        </div>
...
export default function SearchClient({ guides }: { guides: Guide[] }) {
  ...
  return (
    <div style={{ background: "var(--sand)", minHeight: "100vh", fontFamily: "var(--font-body)" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(246,241,232,0.95)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: "1px solid var(--sand-dark)" }}>
        ...
        <div style={{ padding: "0 20px 10px" }}>
          <div style={{ position: "relative", background: "var(--white)", borderRadius: "999px", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", border: "1.5px solid var(--sand-dark)", display: "flex", alignItems: "center", padding: "0 6px 0 18px", marginBottom: 10 }}>
            <span style={{ fontSize: 15, color: "var(--muted)", flexShrink: 0 }}>🔍</span>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Guide, ville, expérience..." style={{ flex: 1, border: "none", outline: "none", padding: "13px 12px", fontSize: 13, fontFamily: "var(--font-body)", color: "var(--charcoal)", background: "transparent" }} />
            {search && <button onClick={() => setSearch("")} style={{ width: 26, height: 26, borderRadius: "50%", background: "var(--sand-dark)", border: "none", color: "var(--soft)", fontSize: 12, cursor: "pointer", flexShrink: 0 }}>✕</button>}
            <div style={{ width: 1, height: 22, background: "var(--sand-dark)", margin: "0 8px" }} />
            <button onClick={() => setOpen(!open)} style={{ background: open ? "var(--bronze-g)" : (hasFilters ? "#FFF8ED" : "var(--sand)"), border: hasFilters && !open ? "1.5px solid var(--bronze)" : "none", borderRadius: "999px", padding: "7px 14px", fontSize: 11, fontWeight: 700, color: open ? "#fff" : (hasFilters ? "var(--bronze)" : "var(--charcoal)"), cursor: "pointer", fontFamily: "var(--font-body)", display: "flex", alignItems: "center", gap: 5, flexShrink: 0 }}>
              ⚙ Filtres {hasFilters && <span style={{ width: 6, height: 6, background: open ? "#fff" : "var(--bronze)", borderRadius: "50%", display: "inline-block" }} />}
            </button>
          </div>
          <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 2, scrollbarWidth: "none" }}>
            {CITIES.map(c => (
              <button key={c} onClick={() => setCity(c === city ? "All" : c)} style={{ padding: "6px 15px", borderRadius: "999px", border: "none", background: city === c ? "var(--bronze-g)" : "var(--white)", color: city === c ? "#fff" : "var(--soft)", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "var(--font-body)", whiteSpace: "nowrap", flexShrink: 0, boxShadow: city === c ? "0 4px 12px rgba(184,138,68,0.35)" : "0 2px 6px rgba(0,0,0,0.05)" }}>
                {c === "All" ? "✦ Toutes" : c}
              </button>
            ))}
          </div>
          ...
          <div style={{ background: "var(--white)", padding: "18px 20px 22px", borderTop: "1px solid var(--sand-dark)" }}>
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {TYPES.map(t => (
                  <button key={t} onClick={() => toggle(t)} style={{ padding: "7px 14px", borderRadius: "999px", border: `1.5px solid ${types.includes(t) ? "var(--sage)" : "var(--sand-dark)"}`, background: types.includes(t) ? "#E8F0E4" : "var(--sand)", color: types.includes(t) ? "var(--sage)" : "var(--soft)", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "var(--font-body)", display: "flex", alignItems: "center", gap: 5 }}>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {LANGS.map(l => (
                  <button key={l} onClick={() => setLang(l === lang ? "All" : l)} style={{ padding: "7px 14px", borderRadius: "999px", border: `1.5px solid ${lang === l && l !== "All" ? "var(--bronze)" : "var(--sand-dark)"}`, background: lang === l && l !== "All" ? "#FFF3E0" : "var(--sand)", color: lang === l && l !== "All" ? "var(--bronze)" : "var(--soft)", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "var(--font-body)" }}>
                    {l}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 18 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <div style={{ fontSize: 10, fontWeight: 800, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 1.5 }}>Budget max</div>
                <span style={{ fontFamily: "var(--font-serif)", fontWeight: 700, fontSize: 16, color: "var(--bronze)" }}>€{maxPrice}</span>
              </div>
              <input type="range" min={30} max={300} step={5} value={maxPrice} onChange={e => setMaxPrice(Number(e.target.value))} style={{ width: "100%", accentColor: "var(--bronze)", cursor: "pointer" }} />
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={reset} style={{ flex: 1, padding: 12, border: "1.5px solid var(--sand-dark)", borderRadius: "999px", background: "var(--white)", fontSize: 12, fontWeight: 600, color: "var(--soft)", cursor: "pointer", fontFamily: "var(--font-body)" }}>Reset</button>
              <button onClick={() => setOpen(false)} style={{ flex: 2, padding: 12, borderRadius: "999px", background: "var(--bronze-g)", color: "#fff", fontSize: 12, fontWeight: 700, border: "none", cursor: "pointer", fontFamily: "var(--font-body)" }}>Voir {filtered.length} guide{filtered.length !== 1 ? "s" : ""} →</button>
            </div>
          </div>
        )}
      </div>
      <div style={{ padding: "20px 20px 80px", maxWidth: 680, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 18 }}>
          <div>
            <h1 style={{ fontFamily: "var(--font-serif)", fontSize: 22, fontWeight: 700, color: "var(--charcoal)", marginBottom: 3, lineHeight: 1.2 }}>{city !== "All" ? `Guides — ${city}` : "Tous les Guides"}</h1>
            <p style={{ fontSize: 11, color: "var(--muted)" }}><strong style={{ color: "var(--charcoal)" }}>{filtered.length}</strong> expert{filtered.length !== 1 ? "s" : ""} certifié{filtered.length !== 1 ? "s" : ""} · Laksor verified</p>
          </div>
          <div style={{ position: "relative" }}>
            <select value={sort} onChange={e => setSort(e.target.value)} style={{ appearance: "none", WebkitAppearance: "none", background: "var(--white)", border: "1.5px solid var(--sand-dark)", borderRadius: "999px", padding: "7px 30px 7px 14px", fontSize: 11, fontWeight: 700, color: "var(--charcoal)", fontFamily: "var(--font-body)", cursor: "pointer", outline: "none" }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 20px", background: "var(--white)", borderRadius: "var(--r-card)" }}>
            ...
            <button onClick={reset} style={{ background: "var(--bronze-g)", color: "#fff", border: "none", borderRadius: "999px", padding: "12px 24px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "var(--font-body)" }}>Effacer les filtres</button>
          </div>
        ) : (
        ...
        {filtered.length > 0 && (
          <div style={{ marginTop: 28, background: "var(--white)", borderRadius: 20, padding: "18px 20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[{ icon: "🛡️", text: "Guides certifiés & vérifiés" },{ icon: "🔄", text: "Annulation gratuite 72h avant" },{ icon: "💬", text: "Support WhatsApp 7j/7" },{ icon: "🚗", text: "Transport en 1 clic" }].map(t => (
